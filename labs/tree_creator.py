...
def createTree(data, attributes, high, overall_tree_high, tree_high, classes, num_attributes, num_classes):
    ...
    if (len(data) == 0):
        ...
        node.children = [createTree(subset, remaining_attributes, high+1, overall_tree_high, tree_high, classes, num_attributes, num_classes) for subset in splitted]
        # if get majority class in both cases are the same
        if (node.children[0].isLeaf == True & node.children[1].isLeaf == True & node.children[1].label == node.children[0].label):
